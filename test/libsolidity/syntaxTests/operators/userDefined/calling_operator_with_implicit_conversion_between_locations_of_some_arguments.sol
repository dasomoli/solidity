...
// ----
// TypeError 5653: (408-413): User-defined binary operator + only accepts type struct S memory and is not compatible with types struct S memory and struct S calldata.
// TypeError 5653: (423-428): User-defined binary operator + only accepts type struct S memory and is not compatible with types struct S memory and struct S storage ref.
...
// TypeError 5653: (511-516): User-defined binary operator - only accepts type struct S calldata and is not compatible with types struct S calldata and struct S memory.
// TypeError 5653: (526-531): User-defined binary operator - only accepts type struct S calldata and is not compatible with types struct S calldata and struct S storage ref.
...
// TypeError 5653: (613-618): User-defined binary operator * only accepts type struct S storage pointer and is not compatible with types struct S storage ref and struct S calldata.
// TypeError 5653: (628-633): User-defined binary operator * only accepts type struct S storage pointer and is not compatible with types struct S storage ref and struct S memory.
